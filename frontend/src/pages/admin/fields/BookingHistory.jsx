...
import { Download, RotateCcw, X, Loader2, Eye } from "lucide-react";
    ...
    adminApi().get("/bookings")
      .then(res => {
        const normalized = (res.data || []).map(b => ({
          ...
          applicant:   { name: b.full_name, dept: b.dept, contact: b.contact_number },
    ...
    <div>
      ...
      <div className="glass" style={{ borderRadius: 18, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} style={{ textAlign: "center", padding: "50px 0", color: "var(--text3)" }}>
                <Loader2 size={18} className="animate-spin" style={{ margin: "0 auto 8px", display: "block" }} /> Loading...
              </td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: "center", padding: "50px 0", color: "var(--text3)" }}>No records found</td></tr>
            ) : filtered.map(b => (
              <tr key={b.id} className="table-row" style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <td style={{ padding: "12px 18px" }}>
                  <button
                    onClick={() => { setSelectedBooking(b); setPreviewOpen(false); }}
                    style={{ padding: "6px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 5, background: "var(--surface2)", border: "1px solid var(--border2)", color: "var(--text2)", cursor: "pointer", transition: "all 0.2s", whiteSpace: "nowrap" }}
                  >
                    <Eye size={13} /> Details
                  </button>
                </td>
      ...
      {/* Detail Modal */}
{selectedBooking && (
  <div className="modal-overlay" onClick={e => e.target === e.currentTarget && (setSelectedBooking(null), setPreviewOpen(false))}>
    {!previewOpen ? (
      <div className="modal-box" style={{ maxWidth: 600 }}>
        <div className="flex items-center justify-between mb-6">
          <h3 style={{ fontWeight: 700, fontSize: 18, color: "var(--text)" }}>Booking Details</h3>
          <span className="badge" style={{ background: "rgba(79,110,247,0.15)", color: "var(--accent2)" }}>{selectedBooking.id}</span>
        </div>

        {/* Status Badge */}
        <div style={{ marginBottom: 20 }}>
          <span className="badge" style={{
            ...{ Approved: { background: "rgba(34,197,94,0.12)", color: "var(--green)", border: "1px solid rgba(34,197,94,0.2)" }, Rejected: { background: "rgba(239,68,68,0.12)", color: "var(--red)", border: "1px solid rgba(239,68,68,0.2)" }, Cancelled: { background: "rgba(249,115,22,0.12)", color: "var(--orange)", border: "1px solid rgba(249,115,22,0.2)" }, Pending: { background: "rgba(234,179,8,0.12)", color: "var(--yellow)", border: "1px solid rgba(234,179,8,0.2)" } }[selectedBooking.status],
        ...
        <div style={{ background: "var(--bg)", borderRadius: 12, padding: 16, marginBottom: 16, border: "1px solid var(--border)" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 12 }}>Event Info</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {[
              { label: "Event Title", value: selectedBooking.title },
              { label: "Spot / Venue", value: selectedBooking.spotName },
              { label: "Date", value: selectedBooking.endDate ? `${selectedBooking.date} → ${selectedBooking.endDate}` : selectedBooking.date },
              { label: "Session", value: selectedBooking.session || "—" },
              { label: "Start Time", value: selectedBooking.startTime || "—" },
              { label: "End Time", value: selectedBooking.endTime || "—" },
            ].map(({ label, value }) => (
              <div key={label}>
        ...
        {/* Applicant Info */}
        <div style={{ background: "var(--bg)", borderRadius: 12, padding: 16, marginBottom: 16, border: "1px solid var(--border)" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 12 }}>Applicant</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {[
              { label: "Full Name",    value: selectedBooking.applicant?.name        || "—" },
              { label: "Department",   value: selectedBooking.applicant?.dept        || "—" },
              { label: "Designation", value: selectedBooking.applicant?.designation  || "—" },
              { label: "Contact",      value: selectedBooking.applicant?.contact     || "—" },
            ].map(({ label, value }) => (
              <div key={label}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>{label}</div>
                <div style={{ fontWeight: 600, color: "var(--text)", fontSize: 13 }}>{value}</div>
        ...
        {selectedBooking.recommender?.name && (
          <div style={{ background: "var(--bg)", borderRadius: 12, padding: 16, marginBottom: 16, border: "1px solid var(--border)" }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 12 }}>Recommender</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {[
                { label: "Full Name",   value: selectedBooking.recommender?.name || "—" },
                { label: "Post",        value: selectedBooking.recommender?.post || "—" },
                { label: "Department",  value: selectedBooking.recommender?.dept || "—" },
              ].map(({ label, value }) => (
                <div key={label}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text3)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 4 }}>{label}</div>
                  <div style={{ fontWeight: 600, color: "var(--text)", fontSize: 13 }}>{value}</div>
        ...
        {selectedBooking.description && (
          <div style={{ background: "var(--bg)", borderRadius: 12, padding: 16, marginBottom: 20, border: "1px solid var(--border)" }}>
            ...
            <div style={{ fontSize: 13, color: "var(--text2)", lineHeight: 1.6 }}>{selectedBooking.description}</div>
          </div>
        )}

        {/* Actions */}
        <div className="flex gap-3">
          <button className="btn-ghost" style={{ flex: 1, padding: "11px", borderRadius: 12, fontWeight: 600 }} onClick={() => setSelectedBooking(null)}>Close</button>
          <button className="btn-primary" style={{ flex: 1, padding: "11px", borderRadius: 12, fontWeight: 600 }} onClick={() => setPreviewOpen(true)}>Preview Document</button>
        </div>
      </div>
      ...
      <div style={{ background: "var(--bg2)", borderRadius: 24, width: "100%", maxWidth: 700, maxHeight: "90vh", display: "flex", flexDirection: "column", border: "1px solid var(--border2)" }}>
          ...
          <div style={{ background: "white", color: "#1a1a2e", padding: 40, borderRadius: 12, fontFamily: "Georgia, serif", minHeight: 600 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
              {[
                ...
                ["Date",        selectedBooking.endDate ? `${selectedBooking.date} to ${selectedBooking.endDate}` : selectedBooking.date],
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20, borderTop: "1px solid #eee", paddingTop: 20, marginBottom: 20 }}>
              {[
                { title: "Applicant",   name: selectedBooking.applicant?.name,   sub: selectedBooking.applicant?.dept,   note: selectedBooking.applicant?.designation },
                { title: "Recommender", name: selectedBooking.recommender?.name, sub: selectedBooking.recommender?.dept, note: selectedBooking.recommender?.post },
        ...
        <div style={{ padding: "14px 24px", background: "var(--bg3)", borderRadius: "0 0 24px 24px", display: "flex", justifyContent: "flex-end", gap: 10 }}>
          <button className="btn-primary" style={{ padding: "9px 24px", borderRadius: 10, fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}
