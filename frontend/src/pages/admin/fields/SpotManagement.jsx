...
import {
  Camera, Check, ImagePlus, X, Trash2, Loader2, Plus,
} from "lucide-react";
...
const SpotManagement = () => {
  ...
  useEffect(() => {
    ...
    adminApi().get(`/spots/${activeSpot.spot_id}/recipients`)
      .then(r => setRecipients(r.data.length
        ? r.data.map(x => ({ recipient_name: x.recipient_designation || "", recipient_email: x.recipient_email || "" }))
